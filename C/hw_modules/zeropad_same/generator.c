...
int main(int argc,char **argv)
{
    ...
    if ((out_file = fopen("OutFile.txt","w")) == NULL){
	...
	for (ii = 0;ii < 3;ii++){
	    ...
	    fprintf(out_file,"%hu\n",(desc_input.dimensions[ii] >> 8));        
	    fprintf(out_file,"%hu\n",(desc_input.dimensions[ii]));
	}
	...
	for (ii = 0;ii < 4;ii++){
	    ...
	    fprintf(out_file,"%hu\n",(desc_kernel.dimensions[ii] >> 8));        
	    fprintf(out_file,"%hu\n",(desc_kernel.dimensions[ii]));
	}
    ...
    for(ii = 0;ii < 2;ii++){
		uint8_t var1,var2;
		fscanf(param_file,"%u",&var1);
		fscanf(param_file,"%u",&var2);
    ...
    if (desc_input.data_type == u8){
		...
		for (ii = 0; ii < kernel_size; ii++)
		{
			...
			else temp[ii&7] = (ii+1)%255;					//Sequential data
			fprintf(out_file,"%hd\n",temp[ii&3]);
		}
	}	
	else{
