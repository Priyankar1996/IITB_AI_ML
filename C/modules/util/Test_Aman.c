...
int main(){
    ...
    for (int i = 0; i < 2*num_iters; i++)
    {
        ...
        T[i].descriptor.number_of_dimensions = 3;
    ...
    for (int i=0; i < 3*num_iters; i++)
    {
        ...
        S[i].descriptor.number_of_dimensions = 3;
        S[i].descriptor.row_major_form = 0;
    }

    K.descriptor.data_type = float32;
    K.descriptor.number_of_dimensions = 3;
    ...
    for (int i = num_iters; i < 2*num_iters; i++){
        ...
        _err_ = dilateTensor(&T[i], &K, stride,  &S[i]) ||
                dePadTensor(&S[i],pad_deconv,&S[2*num_iters + i]) ||
                convTensors(&S[2*num_iters + i],&K,&T[i+1],stride_deconv,pad_deconv ) || _err_;
