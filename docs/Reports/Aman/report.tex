...
We also integrate a cluster of eight engines with a 32-bit AJIT processor and a Network Interface Card to generate an SoC achieving an average throughput of over 300 GOPS and image throughput of 5 fps. Using these results, we show that the performance per unit resource of our engine is comparable with those of the implementations reported in recent literature.
}
...
The primary reason for the degradation is the use of limited memory bandwidth which is being multiplexed by all the engines, which gets intensified due to the high latency of DRAM accesses. To mitigate the issues, we have to reduce either the memory latency or improve the memory bandwidth as seen by the engines. The first one can be obtained by using a cache, which can reduce the latency not only for kernels which are being shared by all the modules, but also the tensors which are accessed serially and benefit from the being loaded from the memory beforehand.
