...
int main(int argc,char **argv)
{
	...
	write_uint8("ConvTranspose_input_pipe",desc_input.row_major_form);
    fscanf(input_file,"%d",&desc_input.number_of_dimensions);
	write_uint8("ConvTranspose_input_pipe",desc_input.number_of_dimensions);
	int ii;
	for (ii = 0;ii < desc_input.number_of_dimensions;ii++){
		fscanf(input_file,"%d",&desc_input.dimensions[ii]);
        write_uint8("ConvTranspose_input_pipe",desc_input.dimensions[ii]);
	}
	...
	write_uint8("ConvTranspose_input_pipe",desc_kernel.row_major_form);
	fscanf(kernel_file,"%d",&desc_kernel.number_of_dimensions);
	write_uint8("ConvTranspose_input_pipe",desc_kernel.number_of_dimensions);

	for (ii = 0;ii < desc_kernel.number_of_dimensions;ii++){
		fscanf(kernel_file,"%d",&desc_kernel.dimensions[ii]);
        write_uint8("ConvTranspose_input_pipe",desc_kernel.dimensions[ii]);
	}
    ...
    for(ii = 0;ii < 3;ii++){
        ...
        write_uint8("ConvTranspose_input_pipe",stride[ii]);
	}
    ...
    if (desc_input.data_type == i16){
		...
		for (ii = 0; ii < input_size; ii++)
		{
			...
			write_uint8("ConvTranspose_input_pipe",temp[ii&3]);	
			write_uint8("ConvTranspose_input_pipe",(temp[ii&3]>>8));
		}
	...
	if (desc_output.data_type == i16){
		...
		for (ii = 0; ii < (size); ii++)
		{
			val = (read_uint8 ("ConvTranspose_output_pipe") << 8) + read_uint8("ConvTranspose_output_pipe");
			fprintf(stderr,"%lu\n",val);		
		}
	}		
	else{
