...
void maxPool3D()
{
	...
	write_uint8("maxpool_output_pipe",rt);
	write_uint8("maxpool_output_pipe",ct);
	write_uint8("maxpool_output_pipe",rb);
	write_uint8("maxpool_output_pipe",cb);
	__aa_barrier__();
	...
	uint16_t re = rb;
	uint16_t dim1d = ct;
	uint16_t offset1 = chl_out>>2, offset2 = dim1d*offset1;
	__aa_barrier__();
#ifndef SW
