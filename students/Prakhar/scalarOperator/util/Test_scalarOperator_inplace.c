...
#include <stdint.h>
#include <math.h>
#include "../src/scalarOperator.c" 
// tensor.h
#include "../../../priyankar/src/createTensor.c" 
// This testbench is for scalarOperator in-place providing random-valued input tensors
...
int main(){
	...
	//call the function
	scalarOperator_inplace(operator, &scalar_value, &a);
	...
	for( ; elements_left > 0; elements_left -= MAX_SIZE_OF_REQUEST_IN_WORDS){
		...
		req1.arguments[1] = b.mem_pool_buffer_pointer+MAX_SIZE_OF_REQUEST_IN_WORDS*iter;
		...
		void *array1;  // contains the from Tensor a i.e. output tensor (same as input tensor)
		array1 = resp.read_data;

		memPoolAccess((MemPool *)(b.mem_pool_identifier),&req1,&resp1); 
		...
		for (int i = 0; i < num_elems ; i++) // again prev was this in place of num_elems: elementsToRead*(8.0/element_size)
		{
			switch(dataType){
				case u8: ; 
					...
					ex_resultu8 = add_scale_uint8(inputu8,operator,(uint8_t) scalar_value);
				...
				case u16: ;
					...
					ex_resultu16 = add_scale_uint16(inputu16,operator,(uint16_t) scalar_value);
				...
				case u32: ;
					...
					ex_resultu32 = add_scale_uint32(inputu32,operator,(uint32_t) scalar_value);
				...
				case u64: ; 
					...
					ex_resultu64 = add_scale_uint64(inputu64,operator,(uint64_t) scalar_value);
					if (resultu64 != ex_resultu64){
						printf("fail at %d, iter = %d Expected Result = %lu Output = %lu  \n",i,iter, ex_resultu64,resultu64);
					}
					else{
				...
				case i8: ; 
					...
					ex_resulti8 = add_scale_int8(inputi8,operator,(int8_t) scalar_value);
				...
				case i16: ;
					...
					ex_resulti16 = add_scale_int16(inputi16,operator,(int16_t) scalar_value);
				...
				case i32: ;
					...
					ex_resulti32 = add_scale_int32(inputi32,operator,(int32_t) scalar_value);
				...
				case i64: ; 
					...
					ex_resulti64 = add_scale_int64(inputi64,operator,(int64_t) scalar_value);
				...
				case float32: ;
					...
					ex_resultf32 = add_scale_f32(inputf32,operator,(float) scalar_value);
				...
				case float64: ;
					...
					ex_resultf64 = add_scale_f64(inputf64,operator,(double) scalar_value);
